(function (root, factory) {
  ...
  function create({ elements, formatters, windowObj = globalThis.window, documentObj = globalThis.document, clock = () => Date.now(), setIntervalImpl = globalThis.setInterval, clearIntervalImpl = globalThis.clearInterval, setTimeoutImpl = globalThis.setTimeout }) {
    let state = { status: 'idle', startedAt: null, json: null, markdown: '', area: null, analysis: null, quality: null, readiness: null, replayPlan: null, replayValidation: null };
    let ticker = null;
    ...
    function render() {
      ...
      if (elements.exportReplay) elements.exportReplay.disabled = !state.replayPlan;
      if (elements.exportReplayValidation) elements.exportReplayValidation.disabled = !state.replayValidation;
      if (elements.exportAudit) elements.exportAudit.disabled = !state.audit;
    ...
    function bind() {
      ...
      elements.exportReplay?.addEventListener('click', () => state.replayPlan && download('devtrail-replay-plan.json', JSON.stringify(state.replayPlan, null, 2), 'application/json;charset=utf-8'));
      elements.exportReplayValidation?.addEventListener('click', () => state.replayValidation && download('devtrail-replay-validation.json', JSON.stringify(state.replayValidation, null, 2), 'application/json;charset=utf-8'));
      elements.exportAudit?.addEventListener('click', () => state.audit && download('devtrail-audit.json', JSON.stringify(state.audit, null, 2), 'application/json;charset=utf-8'));

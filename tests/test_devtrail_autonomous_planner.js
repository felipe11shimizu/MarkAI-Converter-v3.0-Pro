...
(() => {
  const map = { session_id:'s1', target_url:'https://example.test', elements:[
  ...
  const result = planner.plan(map, { maxActions:3 });
  ...
  const observed = planner.plan({ ...map, actions:[{ elemento:'#save' }] }, { maxActions:10, allowInputs:false });
